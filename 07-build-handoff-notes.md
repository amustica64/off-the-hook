...
>
> 1. **`@theme inline`, not `@theme`.** This is load bearing. Without `inline`, Tailwind re-emits the token values into its own variables instead of resolving the references, so every semantic alias that flips under `html[data-theme="dark"]` stops responding to the theme. Dark mode silently half-works. Do not remove `inline`.
